...
class Position(models.Model):
    ...
    wine_position = models.ForeignKey(
        Catalog, on_delete=models.CASCADE, related_name="positions"
    )
    ...
    @staticmethod
    def validate_available_positions(ordered_amount: int, available_amount: int, error):
        if ordered_amount > available_amount:
            raise error(f"There is only {available_amount} left!")

    def clean(self):
        Position.validate_available_positions(
            self.quantity, self.wine_position.amount, ValidationError
        )
